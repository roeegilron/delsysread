...
haxDelAcc      = subplot(nrows,ncols,1);
% haxRcsAcc      = subplot(nrows,ncols,4);
haxRcsSpect    = subplot(nrows,ncols,2);
haxRcsSpectSTN = subplot(nrows,ncols,3);
...
% plot delsys gyro tremo hand 
axes(haxDelAcc); 
hold on; 
xx = dataraw.R_Hand_GyroX2_IM_;
yy = dataraw.R_Hand_GyroY2_IM_;
zz = dataraw.R_Hand_GyroZ2_IM_;
secs = seconds((0:1:length(xx )-1 )./dataraw.srates.Gyro)-params.delsys5Hz; 
plot(haxDelAcc,secs',xx-mean(xx),'LineWidth',2);
plot(haxDelAcc,secs',yy-mean(yy),'LineWidth',2);
plot(haxDelAcc,secs',zz-mean(zz),'LineWidth',2);
...
linkaxes([haxDelAcc, haxRcsSpect, haxRcsSpectSTN],'x');
